...
For this kind of bidirectional communication, the controllingOSC device would have to query and update the parameter values actively and continuously (polling function via OSC). Unfortunately, polling is not possible with DiGiCo Generic OSC.
...
This provides a workflow in which d&b Soundscape can beoperated by a console, the R1 Remote control software and other controllers simultaneously. And the console always "knows" and displays the current parameter state ofthe DS100.
...
*Protocol A* for the connection with the DS100: 
* Enter the IP address of the DS100 in the first field. The IP address of the DS100 can be obtained from the «Info» tab in the «Device» view in R1 V3.
* Enter the Send port and the Receive port divided by a comma in the second field. (the Send port is the listening port of the DS100 = 50010; the Receive port is the reply port of the DS100 = 5001)
* Enable the Active Obj. Handling check mark (to activate polling).

### Connection with the Console
*Protocol B* for the connection with the Console:
* Enter the IP address of the console in the first field. The IP address of the DS100 can be obtained from the «Info» tab in the «Device» view in R1 V3
* Enter the Receive port and the Send port of the console divided by a comma in the second field. (!!! The ports defined in the External Control Config window BUT SWAPPED !!!)
* DO NOT enable the Active Obj. Handling check mark.

*Enabling parameters and channels to be controlled and polled via the Software bridge*
* Open the “General configuration” window by clicking the “Global Configuration” button.

[Picture]

* Enable the parameters you want to use by checking the “active” box to the right of the parameter description.
* Enter the channels you want to control by defining a range from “first channel number” to “last channel number” or, multiple channel numbers divided by commas
* For the x and y position parameters, define the coordinate mapping / positioning area by entering a value between 1 – 4 in the “record” fields. This must be the same value you used for the
“/dbaudio1/coordinatemapping/source_position_x/ & /dbaudio1/coordinatemapping/source_position_y/ “ 
for Generic OSC on the console.
* Finish the configuration of the Software bridge by clicking the OK button in the General configuration window.
...
*Note:* The Software bridge can only handle the following d&b Soundscape object parameters: Position, spread, DelayMode and En-Space Send Gain.
